...
const Form = () => {
  ...
  const Submit = async (data) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        icon: 'success',
        html: 'You can <a href="https://api.whatsapp.com/send?phone=917738182493" target="_blank" style="color: #007bff; text-decoration: none;">WhatsApp us at 8879949404</a>',
  ...
  const formFields = [
    ...
    { name: 'salon_contacts.contact_number', label: 'Your Contact*', type: 'number', validtion: { required: { value: true, message: "Your contact number is required" }, minLength: { value: 10, message: "Contact number must contain atleast 10 digits" }, maxLength: { value: 10, message: "Contact number can max contain 10 digits" } } },
          ...
          <div key={index}>
            {(field.type === 'text' || field.type === 'number') && (
              <div className="relative w-full">
                ...
                {(errors[field.name] || errors["salon_contacts"]) && <p className="text-red-500 text-sm mt-1">{errors[field.name]?.message || errors["salon_contacts"][field.name.split(".").pop()]?.message}</p>}
              </div>
            )}
